...
def addsudo(update: Update, context: CallbackContext) -> str:
    ...
    update.effective_message.reply_text(
        rt + "\nSuccessfully set Path level of {} to Asura!".format(
...
def addsupport(
    ...
    if user_id in DRAGONS:
        rt += "Requested AaO to deomote this Asura Path to Human Path."
    ...
    if user_id in WOLVES:
        rt += "Requested HA to promote this Naraka Path to Human Path."
        data['whitelists'].remove(user_id)
        WOLVES.remove(user_id)
...
def addtiger(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in WOLVES:
        rt += "This user is already a Naraka Path, Promoting to Preta Path."
        data['whitelists'].remove(user_id)
        WOLVES.remove(user_id)
...
def removesudo(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in DRAGONS:
        message.reply_text("Requested AO to demote this user to Civilian")
        DRAGONS.remove(user_id)
        data['sudos'].remove(user_id)
...
def removewhitelist(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in WOLVES:
        message.reply_text("Demoting to a Villager")
        WOLVES.remove(user_id)
        data['whitelists'].remove(user_id)
...
def devlist(update: Update, context: CallbackContext):
    ...
    reply = "<b>The Akatsuki Organization Aka Animal Paths:</b>\n"
...
Group admins/group owners do not need these commands. 
 ...
 ╠ `/asurapaths`*:* Lists all Asura Paths
 ╠ `/humanpaths`*:* Lists all Human Paths
 ╠ `/pretapaths`*:* Lists all Preta Paths
 ╠ `/narakapaths`*:* Lists all Naraka Paths
...
SUDO_HANDLER = CommandHandler(("addsudo", "addasura"), addsudo)
SUPPORT_HANDLER = CommandHandler(("addsupport", "addhuman"), addsupport)
TIGER_HANDLER = CommandHandler(("addpreta"), addtiger)
WHITELIST_HANDLER = CommandHandler(("addwhitelist", "addnaraka"), addwhitelist)
UNSUDO_HANDLER = CommandHandler(("removesudo", "removeasura"), removesudo)
UNSUPPORT_HANDLER = CommandHandler(("removesupport", "removehuman"),
                                   removesupport)
UNTIGER_HANDLER = CommandHandler(("removepreta"), removetiger)
UNWHITELIST_HANDLER = CommandHandler(("removewhitelist", "removenaraka"),
                                     removewhitelist)

WHITELISTLIST_HANDLER = CommandHandler(["whitelistlist", "narakapaths"],
                                       whitelistlist)
TIGERLIST_HANDLER = CommandHandler(["pretapaths"], tigerlist)
SUPPORTLIST_HANDLER = CommandHandler(["supportlist", "humanpaths"], supportlist)
SUDOLIST_HANDLER = CommandHandler(["sudolist", "asurapaths"], sudolist)
DEVLIST_HANDLER = CommandHandler(["devlist", "animals", "akatsuki"], devlist)
